(function attachManualUpdateModal(global) {
  ...
  global.BOSS_COMPONENTS.renderManualUpdateModal = function renderManualUpdateModal(options) {
    ...
    var resolvedMode = allowUpload ? options.mode : "pull";
    var disableSubmit = options.canSubmit === false;
    var uploadHintHtml =
      options.uploadHint || options.uploadRuleLink
        ? '<div class="form-hint form-hint-row">' +
          (options.uploadHint ? '<span>' + escapeHtml(options.uploadHint) + "</span>" : "<span></span>") +
          (options.uploadRuleLink
            ? '<a class="form-hint-link" href="' +
              escapeHtml(options.uploadRuleLink.href || "#") +
              '" target="_blank" rel="noreferrer">' +
              escapeHtml(options.uploadRuleLink.label || "查看命名规则") +
              "</a>"
            : "") +
          "</div>"
        : "";
    ...
    var pullHtml =
      ...
      "</div></div>";
    var bodyFields = resolvedMode === "upload" ? agentMonthHtml + uploadHtml : agentMonthHtml + pullHtml;
    var uploadModeHtml = allowUpload
